window.onload = function() {
    ...
    formulario.addEventListener("submit", function(event) {
        ...
        if (fieldName.value.length < 2) {
            errores.push({ param: "name", msg: "FRONT Ingrese un nombre mayor a 2 caracteres" })
        }

        let fieldSurname = document.querySelector("input#surname");

        if (fieldSurname.value.length < 2) {
            errores.push({ param: "surname", msg: "FRONT Ingrese un apellido mayor a 2 caracteres" })
        }

        let fieldEmail = document.querySelector("input#email");

        if (fieldEmail.value.length == "") {
            errores.push({ param: "email", msg: "FRONT Ingrese un email" })
        }

        let fieldUsername = document.querySelector("input#username");

        if (fieldUsername.value == "") {
            errores.push({ param: "username", msg: "FRONT El campo no puede estar vacío" })
        } else if (fieldUsername.value.length < 4) {
            errores.push({ param: "username", msg: "FRONT El username debe tener al menos 4 caracteres" })
        }

        let fieldPassword = document.querySelector("input#password");

        if (fieldPassword.value.length < 8) {
            errores.push({ param: "password", msg: "FRONT La contraseña debe tener al menos 8 caracteres" })
        }

        let fieldRepassword = document.querySelector("input#repassword");

        if (fieldRepassword.value == "") {
            errores.push({ param: "repassword", msg: "FRONT Confirme contraseña" })
        } else if (fieldRepassword.value != fieldPassword.value) {
            errores.push({ param: "repassword", msg: "FRONT Ambas contraseñas deben coincidir" })
        }
